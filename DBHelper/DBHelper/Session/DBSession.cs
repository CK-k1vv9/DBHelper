...
namespace DBUtil
{
    public class DBSession : ISession
    {
        ...
        public void Insert<T>(List<T> list)
        {
            ...
            {
                Insert<T>(list.Skip(i).Take(500).ToList(), _autoIncrement);
            }
        }
        ...
        public async Task InsertAsync<T>(List<T> list)
        {
            for (int i = 0; i < list.Count; i += 500)
            {
                await InsertAsync<T>(list.Skip(i).Take(500).ToList(), _autoIncrement);
            }
        }
        ...
        /// 批量添加
        /// </summary>
        public void Insert<T>(List<T> list, bool autoIncrement)
        {
            StringBuilder strSql = new StringBuilder();
            int savedCount = 0;
            DbParameter[] parameters = null;

            PrepareInsertSql<T>(list, autoIncrement, ref strSql, ref parameters, ref savedCount);
        ...
        /// 批量添加
        /// </summary>
        public async Task InsertAsync<T>(List<T> list, bool autoIncrement)
        {
            StringBuilder strSql = new StringBuilder();
            int savedCount = 0;
            DbParameter[] parameters = null;
